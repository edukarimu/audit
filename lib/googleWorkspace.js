...
const SHEET_TABS = {
  audits: {
    ...
    header: [
      ...
      "Ward", "Village", "Sub-Village", "Asset Tag", "Asset Type",
  ...
  findings: {
    title: "Findings",
    header: [
      "Synced at", "Audit ID", "School", "Unit",
      "Ward", "Village", "Sub-Village", "Asset Tag",
      "Section", "Group", "Statement", "Note", "Photo links",
    ],
  },
};
...
export async function syncAuditToWorkspace(audit) {
  ...
  for (const item of issues) {
    ...
    findingRows.push([
      ...
      audit.ward || "", audit.village || "", audit.subVillage || "", audit.assetTag || "",
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      values: [[
        ...
        audit.ward || "", audit.village || "", audit.subVillage || "", audit.assetTag || "", audit.assetType || "",
  ...
  const PHOTO_LINKS_COL = 12; // keep in sync with the Findings header order above
  return { syncedAt, findingsWritten: findingRows.length, photosUploaded: findingRows.reduce((n, r) => n + (r[PHOTO_LINKS_COL] ? r[PHOTO_LINKS_COL].split(", ").length : 0), 0) };
}
